...
public class ReviewDao {
	...
	public ReviewDao() {
		String path = ReviewDao.class.getResource("/sql/review.properties").getPath();
		try {
	...
	private Review getReview(ResultSet rs) throws SQLException {
		return Review.builder().reviewId(rs.getString("review_id")).paymentId(rs.getString("payment_id"))
				.reviewTitle(rs.getString("review_title")).reviewContent(rs.getString("review_content"))
				.reviewRating(rs.getInt("review_Rating")).reviewDate(rs.getDate("review_date"))
				.imgPath(rs.getString("CLASS_THUMBNAIL")).memberId(rs.getString("member_id"))
				.classTitle(rs.getString("class_title")).memberNickname(rs.getString("member_nickname")).build();
	}
