...
public class ReservationScheduleServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Payment> paymentList=new PaymentService().selectPaymentByhostId(hostId);
		//String a =rsv.get(0);
		List<Classes> classList=new PaymentService().selectClassByhostId(hostId);
		
		
		System.out.println(paymentList);
		request.setAttribute("paymentList",paymentList);
		request.setAttribute("classList",classList);
		request.getRequestDispatcher("/views/host/hostReservation.jsp").forward(request, response);
		...
		//gson.toJson(paymentList,response.getWriter()); // list 보낼 때는 바로 list 보내도 됨 ㅠㅠ
