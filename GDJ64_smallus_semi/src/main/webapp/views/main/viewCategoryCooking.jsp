...
<%
	...
	List<ClassIndex> allCooking=(List)request.getAttribute("allCooking");
%>
<style>
div.craft{
display: felx;
...
<section class="h-categoryMain">
	<img src="<%=request.getContextPath()%>/img/category_beauty.jpg" alt="re1" title="내 손으로 작은 바..."
...
<div class="h-main">
	...
	<section class="h-imgContainer">
			<%
	int wcounts2 = 0;
	if (allCooking != null && !allCooking.isEmpty()) {
		for (ClassIndex m : allCooking) {
			boolean isWish = false;
			if (loginMember != null) {
		List<Wish> wish = (List<Wish>) session.getAttribute("wishMember");
		for (Wish w : wish) {
