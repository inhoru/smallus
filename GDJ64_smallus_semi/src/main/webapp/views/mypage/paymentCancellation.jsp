            ...
            <section class="i-mypageCommonbackground">
                <div class="i-mypageCommon">
                    ...
                    <div class="i-paymentTypeDate i-completed">
                        <span><%=p.getPayment().getPaymentStatus()%> </span><span>2023.05.19</span><span> <%=p.getPayment().getPaymentDate()%></span><br />
                    </div>
                    ...
                    <div class="i-paymentInformation">
                        <h3>취소정보</h3>
                        <div class="i-paymentInformationDetails">
                            <span>환불요청 금액</span><span><%=p.getClasses().getClassPrice()*p.getPayment().getClassPersonnel()%>원</span>
                        </div>
                        <div class="i-paymentInformationDetails">
                            <span>쿠폰 공제액</span><span>-<%=p.getCoupon().getCouponPrice()%>원</span>
                        </div>
                        <div class="i-paymentInformationDetails">
                            <span>총 결제금액</span><span><%=p.getPayment().getTotalPrice()%>원</span>
                        </div>
                    </div>
                    <div class="i-paymentInformation i-Cancellation">
                        <h3>환불정보</h3>
                        <div class="i-paymentInformationDetails">
                            <span><%=p.getPaymentType().getPaymentName()%>결제</span><span><%=p.getPayment().getTotalPrice()%>원</span>
                        </div>
                        <div class="i-paymentInformationDetails">
                            <span>총 환불 금액</span><span><%=p.getPayment().getTotalPrice()%>원</span>
                        </div>
